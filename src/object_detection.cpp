...
// ROI parameters
double zMinROI = -0.5, zMaxROI = 2.0, xMinROI = 0.0, xMaxROI = 4.0, yMinROI = -3.0, yMaxROI = 3.0;
...
int minPoints = 10;
double epsilon = 0.1, minClusterSize = 20, maxClusterSize = 2000;
// VoxelGrid parameter
...
void cloud_cb(const sensor_msgs::PointCloud2ConstPtr& cloud_msg) {
    ...
    for (const auto& indices : cluster_indices) {
        ...
        if (width >= xMinBoundingBox && width <= xMaxBoundingBox &&
            ...
            marker.id = detected_markers.size();
    ...
    marker_array.markers.reserve(MAX_MARKERS);
    ...
    for (const auto& tracked_obj : tracked_objects) {
        ...
        marker.scale.x = std::max(marker.scale.x, 0.2);  // Minimum width
        marker.scale.y = std::max(marker.scale.y, 0.2);  // Minimum depth
        marker.scale.z = std::max(marker.scale.z, 0.2);  // Minimum height
...
int main(int argc, char** argv) {
    ...
    ROS_INFO("Human detection node initialized.");
    ROS_INFO("ROI: x[%.2f, %.2f], y[%.2f, %.2f], z[%.2f, %.2f]", xMinROI, xMaxROI, yMinROI, yMaxROI, zMinROI, zMaxROI);
    ROS_INFO("Bounding box: x[%.2f, %.2f], y[%.2f, %.2f], z[%.2f, %.2f]", xMinBoundingBox, xMaxBoundingBox, yMinBoundingBox, yMaxBoundingBox, zMinBoundingBox, zMaxBoundingBox);
